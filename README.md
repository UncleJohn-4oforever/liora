...
**Version: 0.10.2** — [CHANGELOG](./CHANGELOG.md) · [Versioning](./docs/VERSIONING.md) · [Long-chat QA](./docs/QA_LONG_CHAT.md) · [Character QA](./docs/QA_CHARACTER.md) · [Vision](./docs/VISION.md)
...
### Windows release (0.10.2)

After `npm run tauri build`:

`src-tauri/target/release/bundle/nsis/Liora_0.10.2_x64-setup.exe`

Attach that file to a GitHub Release (`v0.10.2`). Do **not** commit `node_modules/`, `src-tauri/target/`, or user data.
...
```bash
npm install
npm run version:sync -- 0.10.2   # when releasing
npm run tauri build
```
